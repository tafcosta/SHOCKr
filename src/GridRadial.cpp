...
void GridRadial::update(double dt) {
	...
	for(int i = minXIndex; i <= maxXIndex; i++){
		...
		pLeft  = (static_cast<EquationsEuler*>(&equations))->getPressure(quantities[i-1][EquationsEuler::ENERGY], rhoV2);
		pRight = (static_cast<EquationsEuler*>(&equations))->getPressure(quantities[i+1][EquationsEuler::ENERGY], rhoV2);
		...
		/*
		if(i == 100)
			std::cout << 3 * 2./dVolume * dx * getX(i) * p << " " << wellBalancedSource(3 * 2./dVolume * dx * getX(i-1) * pLeft, 3 * 2./dVolume * dx * getX(i) * p, 3 * 2./dVolume * dx * getX(i+1) * pRight, i) << std::endl;
*/
...
double GridRadial::wellBalancedSource(double sLeft, double sMid, double sRight, int i){
	...
	p     = (static_cast<EquationsEuler*>(&equations))->getPressure(quantities[i][EquationsEuler::ENERGY], rho * std::pow(vx, 2.));
	...
	for(int j = 0; j <= 2; j++)
		res += 3 / dx * (AconsMinus[1][j] * (deltaMid[j] + deltaRight[j]) * std::pow(getX(i) + dx / 2., 2.) + AconsPlus[1][j] * (deltaMid[j] + deltaLeft[j]) * std::pow(getX(i) - dx / 2., 2.));
