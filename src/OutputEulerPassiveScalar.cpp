...
void OutputEulerPassiveScalar::makeOutput(const std::string& filename){
    ...
    for (int i = 0; i <= grid.maxXIndex; i++) {
        ...
        outputFile << grid.getX(i) << " " << grid.quantities[i][EquationsEulerPassiveScalar::DENS] << " " << grid.quantities[i][EquationsEulerPassiveScalar::XMOM]/grid.quantities[i][EquationsEulerPassiveScalar::DENS] << " " << p << " " << v/std::sqrt(5./3*p/grid.quantities[i][EquationsEulerPassiveScalar::DENS]) << std::endl;
    }
